...
|**Фильтр**|**Результат**|
|---|---|
|ведущий|`host` отфильтрует видимый трафик, чтобы отобразить все, что связано с указанным хостом. Двунаправленный|
|src / dest|`src` и `dest` — это модификаторы. С их помощью можно указать исходный или целевой хост или порт.|
|чистый|`net` покажет нам весь трафик, исходящий из указанной сети или направляющийся в неё. Используется обозначение / .|
|прото|будет выполнять фильтрацию по определённому типу протокола. (например, Ethernet, TCP, UDP и ICMP)|
|портвейн|`port` является двунаправленным. Он покажет любой трафик, в котором указанный порт является источником или пунктом назначения.|
|портрандж|`portrange` позволяет указать диапазон портов. (0–1024)|
|меньше / больше "< >"|`less` `greater` можно использовать для поиска пакета или параметра протокола определённого размера.|
|и / &&|`and` `&&` можно использовать для объединения двух разных фильтров. Например, хост src и порт.|
|или|`or` допускает совпадение по одному из двух условий. Необязательно, чтобы совпадало по обоим. Это может быть непросто.|
|нет|`not` Это модификатор, который означает что угодно, кроме x. Например, не UDP.|
